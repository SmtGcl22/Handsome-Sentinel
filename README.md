...
Whenever motion is detected, this camera not only sends you a photo of the intruder but also serves them some fierce lines from the famous "Yakışıklı Güvenlik" (Handsome Security) meme.

## 🌟 Features
- **PIR Motion Detection**: Uses an HC-SR501 PIR sensor to detect motion.
- **Smart Calibration**: Ignores the sensor's unstable boot-up signals for the first 20 seconds.
- **Noise Filter (Debounce)**: Validates motion to prevent false positives caused by wire interference or voltage drops.
- **Spam Protection**: Enforces a strict cooldown between captures so your Telegram inbox doesn't explode.
...
- **Meme Responses**: Replies to intrusions with randomly selected lyrics from "Yakışıklı Güvenlik"!
...
|---|---|
| GPIO 13 | PIR Sensor Data Pin |
| 5V / GND | PIR Sensor Power |

## 🚀 Setup
1. Clone the repository.
2. Open `SecurityCam.ino` in the Arduino IDE.
